...
- Outbound access to the Capgemini model API over WebSocket (`wss://`) and the
  embeddings API over HTTPS
- A valid Capgemini Generative Engine API key (the same key authenticates
  both the WebSocket chat connection and the HTTPS embeddings requests)
...
- Receive model responses incrementally: the backend streams tokens from the
  Capgemini model over a WebSocket connection and relays them to the browser
  as server-sent events (SSE) over HTTP.
